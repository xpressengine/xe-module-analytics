...
class analyticsView extends analytics
{
	...
	function dispAnalyticsAPIData()
	{
		...
		if(!$xml_obj || $xml_obj->response->error->body != 0)
		{
			return new Object(-1, 'msg_invalid_request');
		}

		if(!method_exists($this, '_parse'.ucfirst($method)))
		{
			return new Object(-1, 'msg_undefined_method');
		}
	...
	function _parseVisitPageView($xml_obj)
	{
		...
		else
		{
			foreach($response_obj->data as $key => $val)
			{
				$parsed_data->data[date('YmdHis', strtotime($val->day->body))] = $val->pv->body.'|'.$val->pv_per_visit->body.'|'.$val->newvisit_pv->body.'|'.$val->revisit_pv->body;
	...
	function _parseVisitTime($xml_obj)
	{
		...
		else
		{
			foreach($response_obj->data as $key => $val)
			{
				$parsed_data->data[date('YmdHis', strtotime($val->day->body.'-'.$val->timezone->body))] = $val->sumvisit_count->body.'|'.$val->sumvisitor->body.'|'.$val->sumrevisitor->body.'|'.$val->sumnewvisitor->body.'|'.$val->sumpv->body;
	...
	function _parseVisitDay($xml_obj)
	{
		...
		else
		{
			foreach($response_obj->data as $key => $val)
			{
				$parsed_data->data[date('YmdHis', strtotime($val->day->body))] = $val->avgvisit->body.'|'.$val->avguv->body.'|'.$val->avgnewvisit->body.'|'.$val->avgrevisit->body.'|'.$val->avgpv->body;
	...
	function _parseVisitBack($xml_obj)
	{
		...
		if(!$response_obj->data)
		{
			return new Object('msg_none_data');
		}
